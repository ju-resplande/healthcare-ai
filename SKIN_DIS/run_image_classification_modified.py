...
def main():
    ...
    def eval_step(params, batch):
        ...
        metrics = jax.lax.pmean(metrics, axis_name="batch")
        metrics["predictions"] = jnp.argmax(logits, axis=-1)
    ...
    for epoch in epochs:
        ...
        # ======================== Evaluating ==============================
        eval_metrics = []
        predictions_labels = {"predictions": [], "labels": []}
        ...
        for batch in eval_loader:
            ...
            for info in ["predictions", "labels"]:
                predictions_labels[info].extend(metrics[info])
        ...
        predictions_path = os.path.join(training_args.output_dir, "predictions.json")
        with open(predictions_path, "w") as f:

            for info in ["predictions", "labels"]:
                predictions_labels[info] = jnp.concatenate(
                    predictions_labels[info]
                ).tolist()

            json.dump(predictions_labels, f)
